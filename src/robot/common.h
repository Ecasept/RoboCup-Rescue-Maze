...
// Nearest the robot may drive to a wall to prevent damage. If closer than this, any advnace is stopped. (in cm)
#define US_DAMAGE_PREVENTION_THRESHOLD 8
...
// The left side is slower, so to compensate for that, make it a bit faster by default (in same as MOTOR_SPEED)
#define LEFT_COMPENSATION 0
// If a virtual maze is being simulated for testing
#define TESTING_VIRTUAL false
// time an action takes (turning, advancing) when testing with virtual maze (in ms)
#define VIRT_DELAY 2000
// tile width (in cm)
#define TILE_SIZE 30
// minimum average inclination that a ramp needs to be recognized as one (in degrees)
...
// how many times the robot is allowed to autocorrect per advance() until it is deemed stuck
#define MAX_TURN_TO_CALLS 7
